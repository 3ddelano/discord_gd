...
signal shard_ready(id)
signal shard_resume(id)
signal shard_disconnect(id)
...
#	first_shard_id = 0,
	...
	intents = DiscordConstants.IntentsAllNonPrivileged,
	large_threshold = 250,
...
var user # [ExtendedUser] The bot user
...
var private_channels = DiscordCollection.new(load("res://addons/discord_gd/structures/Channel/PrivateChannel.gd"))
var guild_shard_map = {}
	...
	if typeof(options.get("intents")) == TYPE_ARRAY:
		...
		for intent in options.intents:
			if typeof(intent) in [TYPE_REAL, TYPE_INT]:
				bitmask |= intent
			elif DiscordConstants.Intents[intent]:
				bitmask |= DiscordConstants.Intents[intent]
			else:
				DiscordUtils.perror("DisordClient:_init:Invalid intent: %s" % intent)
		options.intents = bitmask

	print("reached")
	if options.get_all_users and not (options.intents & DiscordConstants.Intents.GUILD_MEMBERS):
		DiscordUtils.perror("DiscordClient:_init:Cannot request all members without GUILD_MEMBERS intent")
...
# @returns [PrivateChannel] | [HTTPResponse] if error
func get_dm_channel(p_user_id):
	...
	return load("res://addons/discord_gd/structures/Channel/PrivateChannel.gd").new(data, self)
...
func get_emoji_guild(p_emoji_id):
	...
	return load("res://addons/discord_gd/structures/Guild.gd").new(data, self)
